...
class Equipment(AuditMixin, Base):
    ...
    last_maintenance_at: Mapped[datetime | None] = mapped_column(Date, nullable=True)
    qr_token: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    remarks: Mapped[str | None] = mapped_column(Text, nullable=True)

    equipment_type: Mapped["EquipmentType | None"] = relationship(back_populates="equipment")
...
class FaultCode(TimestampMixin, Base):
    ...
    equipment_type: Mapped["EquipmentType | None"] = relationship(back_populates="fault_codes")

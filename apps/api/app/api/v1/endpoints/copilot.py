...
def diagnose(payload: DiagnoseRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    wo = None
    if payload.work_order_id:
        wo = db.get(WorkOrder, payload.work_order_id)
    return ai_diagnose(db, payload.fault_description, payload.equipment_id, payload.fault_code, wo, user.id)


@router.post("/rewrite-maintenance-log", response_model=RewriteLogResult)
def rewrite_log(payload: RewriteLogRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return ai_rewrite(db, payload.content, user.id)


@router.post("/generate-report", response_model=MaintenanceReport)
def generate_report(payload: GenerateReportRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def ask(payload: AskRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return ai_ask(db, payload.question, payload.equipment_type_id, payload.fault_code, user.id)


@router.get("/equipment-history/{equipment_id}", response_model=HistorySummary)
def equipment_history(equipment_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):

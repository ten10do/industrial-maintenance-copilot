...
def audit_processed_dataset(
    ...
    *,
    ...
    return {
        ...
        "identity_leakage": not any(intersections.values()),
        "source_leakage": not cross_split_sources,
        "adjacent_window_leakage": duplicate_source_windows == 0,
        "label_leakage": not forbidden,
        "preprocessing_leakage": not any(
    ...
    for name, title in (("paderborn", "Paderborn"), ("xjtu-sy", "XJTU-SY")):
        ...
        lines.extend(
            [
                ...
                f"- Identity leakage check: {'PASS' if item.get('identity_leakage') else 'FAIL'}",
                f"- Source/window leakage check: {'PASS' if item.get('source_leakage') and item.get('adjacent_window_leakage') else 'FAIL'}",
                f"- Label leakage check: {'PASS' if item.get('label_leakage') else 'FAIL'}",
                f"- Preprocessing policy: {'PASS' if item.get('preprocessing_leakage') else 'FAIL'} (scaler/model train-only; selection validation-only; test final-only)",

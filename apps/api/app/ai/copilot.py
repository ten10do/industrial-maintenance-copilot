...
def parse_fault(db: Session, text: str, user_id: int | None = None) -> ParseFaultResult:
    ...
    record_ai_interaction(
        db, user_id, "parse_fault", text, str(parsed.dict()), True, "mock"
    )
    return parsed
...
def _rule_based_parse(db: Session, text: str) -> ParseFaultResult:
    ...
    # 停机
    is_downtime = any(
        k in text for k in ["停机", "停了", "无法运行", "不能启动", "停止"]
    )
    affects_production = any(
        k in text for k in ["整条", "产线", "生产线", "影响生产", "停工"]
    )
    has_safety_risk = any(
        k in text for k in ["异响", "冒烟", "漏电", "火花", "高温", "泄漏", "爆炸"]
    )
    urgency = (
        UrgencyEnum.critical
        if (is_downtime and affects_production)
        else UrgencyEnum.high
        if has_safety_risk
    ...
    for kw in [
        ...
        if kw in text:
            ...
            break
    # 匹配设备编号
    title = f"{eq_keyword or '设备'}故障" + (
        f"-E{fc_match.group(1)}" if fc_match else ""
    )
    return ParseFaultResult(
        title=title,
        description=text,
...
def diagnose(
    ...
    if llm_client.enabled:
        prompt = _build_diagnose_prompt(
            db, fault_description, equipment_id, fault_code, similar
        )
        resp = llm_client.chat(prompt, json_mode=True)
        if resp["content"]:
            data = llm_client.parse_json(resp["content"])
...
def _mock_diagnose(
    fault_description: str, fault_code: str | None, similar: list[SimilarCase]
) -> DiagnoseResult:
    text = fault_description or ""
    causes = []
    if fault_code:
        causes.append(
            {"cause": f"{fault_code} 故障代码相关传感器或驱动异常", "confidence": 0.7}
        )
    if "异响" in text:
...
def rewrite_log(
    db: Session, content: str, user_id: int | None = None
) -> RewriteLogResult:
    if llm_client.enabled:
        prompt = [
            {
                "role": "system",
                "content": "你是维修记录规范化助手。将口语化维修记录改写为规范、专业、客观的记录，保留关键事实，不要添加臆测内容。只输出改写后的文本。",
            },
        ...
        if resp["content"]:
            ...
            return RewriteLogResult(
                original=content, polished=resp["content"], is_mock=False
            )
        record_ai_interaction(
            db,
            user_id,
...
def generate_report(
    ...
    if not wo:
        return MaintenanceReport(
            work_order_id=wo_id, work_order_code="", summary="工单不存在", is_mock=True
        )
    eq_name = ""
    if wo.equipment_id:
    ...
    sections = [
        ReportSection(
            title="故障概述",
            content=f"{wo.title}。{wo.fault_description or ''}".strip(),
        ),
        ReportSection(
            title="影响范围",
            content=f"设备：{eq_name}。停机影响：{'是' if wo.safety_risk else '否'}。",
        ),
        ...
        ReportSection(
            title="安全措施", content=wo.safety_risk or "执行了断电与上锁挂牌"
        ),
        ReportSection(
            title="后续建议", content=wo.follow_up_advice or "建议持续观察设备运行状态"
        ),
    ]
    ...
    record_ai_interaction(
        db, user_id, "generate_report", str(wo_id), summary, True, "mock"
    )
    return MaintenanceReport(
        work_order_id=wo.id,
        work_order_code=wo.code,
...
def _build_report_prompt(wo, eq_name, sections) -> list[dict]:
    ...
    return [
        {
            "role": "system",
            "content": "你是维修报告生成助手。根据工单数据生成结构化维修报告 JSON，包含 sections(数组,每项含title和content) 与 summary 字段。客观严谨，不要编造数据。只返回 JSON。",
        },
        {
            "role": "user",
            "content": f"工单号：{wo.code}\n设备：{eq_name}\n基础数据：\n{base}",
        },
    ]
...
def ask(
    ...
    for s in raw_sources:
        citations.append(
            Citation(
                ...
                or (s["content"][:200] if s.get("content") else None),
    ...
    kb_context = "\n\n".join(
        [
            f"【{s['title']}】(来源:{s['source'] or '内部知识库'}，得分:{s['score']})\n{s['content'][:800]}"
            for s in raw_sources
        ]
    )
    ...
    if llm_client.enabled and raw_sources:
        prompt = [
            {
                "role": "system",
                "content": "你是维修知识问答助手。优先依据提供的知识库内容回答，引用来源。不确定时明确说明。严禁伪造设备手册内容。",
            },
            {
                "role": "user",
                "content": f"知识库参考：\n{kb_context}\n\n问题：{question}",
        ...
        record_ai_interaction(
            db, user_id, "ask", question, None, True, resp["model"], 0, resp["error"]
        )
...
def _build_safety_warnings(question: str) -> list[str]:
    ...
    for kw, msg in risk_keywords.items():
        if (kw in question or kw in lower_q) and msg not in warnings:
            warnings.append(msg)
    return warnings
...
def history_summary(db: Session, equipment_id: int) -> HistorySummary:
    ...
    wos = (
        db.query(WorkOrder)
        .filter(WorkOrder.equipment_id == equipment_id)
    ...
    frequent = [
        {"fault": k, "count": v}
        for k, v in sorted(fault_count.items(), key=lambda x: -x[1])[:5]
    ]
    recent = [
        {
            "code": w.code,
    ...
    common_parts = [
        {"part": k, "count": v}
        for k, v in sorted(parts.items(), key=lambda x: -x[1])[:5]
    ]

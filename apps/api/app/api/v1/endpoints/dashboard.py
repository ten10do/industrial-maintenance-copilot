...
def supervisor_dashboard(db: Session = Depends(get_db), _=Depends(supervisor_or_admin)):
    ...
    pending_dispatch = (
        db.query(WorkOrder)
        .filter(WorkOrder.status == WorkOrderStatusEnum.pending_dispatch)
        .count()
    )
    in_progress = (
        db.query(WorkOrder)
    ...
    pending_acceptance = (
        db.query(WorkOrder)
        .filter(WorkOrder.status == WorkOrderStatusEnum.pending_acceptance)
        .count()
    )
    overdue = (
        db.query(WorkOrder)
        .filter(WorkOrder.planned_end_at < func.now())
        .filter(
    ...
    today_new_faults = (
        db.query(FaultReport).filter(FaultReport.created_at >= today_start).count()
    )
    today_completed = (
        db.query(WorkOrder)
        .filter(
            WorkOrder.status == WorkOrderStatusEnum.completed,
            WorkOrder.actual_end_at >= today_start,
        )
        .count()
    )
    # 平均修复时长
    completed = (
        ...
        .filter(
            WorkOrder.status == WorkOrderStatusEnum.completed,
            WorkOrder.actual_start_at.isnot(None),
            WorkOrder.actual_end_at.isnot(None),
        )
        .all()
    )
    avg_hours = None
    if completed:
        total_h = sum(
            (w.actual_end_at - w.actual_start_at).total_seconds() / 3600
            for w in completed
        )
    ...
    # 优先级分布
    prio_rows = (
        db.query(WorkOrder.priority, func.count(WorkOrder.id))
        .filter(
            WorkOrder.status.notin_(
                [WorkOrderStatusEnum.completed, WorkOrderStatusEnum.cancelled]
            )
    ...
    by_priority = [
        {"priority": p.value if p else "P3", "count": c} for p, c in prio_rows
    ]
    # 设备类型分布
...
def technician_dashboard(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    today_todo = mine.filter(
        WorkOrder.status.in_(
            [WorkOrderStatusEnum.assigned, WorkOrderStatusEnum.accepted]
        )
    ).count()
    high_priority = (
        mine.filter(WorkOrder.priority.in_([PriorityEnum.P1, PriorityEnum.P2]))
        .filter(
            WorkOrder.status.notin_(
                [WorkOrderStatusEnum.completed, WorkOrderStatusEnum.cancelled]
            )
    ...
    overdue = (
        mine.filter(WorkOrder.planned_end_at < func.now())
        .filter(
            WorkOrder.status.notin_(
                [WorkOrderStatusEnum.completed, WorkOrderStatusEnum.cancelled]
            )
    ...
    recent_logs = (
        ...
        .filter(MaintenanceLog.operator_id == user.id)
